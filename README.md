...
对于不熟悉命令行的 Windows 用户，项目根目录提供了 `scripts/one_click_start.bat`：

1. 确保已经安装 **Python 3.10 及以上版本**，安装时勾选 “Add Python to PATH”。
2. 如果计划使用通义千问等大模型，请先按照上文设置好 `DASHSCOPE_API_KEY` 等环境变量。
3. 将体检报告文件复制到 `data/` 文件夹。
4. 双击运行 `scripts/one_click_start.bat`，脚本会自动完成以下任务：
   - 创建/复用 `.venv` 虚拟环境；
   - 安装或更新 `requirements.txt` 中的依赖；
   - 调用 `main.py` 批量生成 HTML 分析报告。

脚本顶部的配置区允许修改数据目录、输出目录、热线电话以及是否启用大模型。如果需要针对不同用户设置热线、咨询电话或指定其他模板，可直接编辑该脚本保存即可。
